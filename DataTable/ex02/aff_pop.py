...
def millions(nums: list) -> list:
    ...
    return [float(i[:-1]) * 1000 if i[-1] == 'k'
            else float(i[:-1]) * 1000000 for i in nums]
...
def aff_pop(csv: DataFrame) -> None:
    ...
    try:
        ...
        years = [int(i) for i in csv.columns]
        plt.plot(years[:-50], list(turkey)[:-50], label='Turkey')
        italy = csv.loc['Italy']
        italy = millions(italy.tolist())
        years = [int(i) for i in csv.columns]
        plt.plot(years[:-50], list(italy)[:-50], label='Italy')
